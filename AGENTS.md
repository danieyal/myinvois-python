...
- A credit note is byte-identical to an invoice apart from **two characters**. A document carrying the wrong type code is well-formed and passes our own validation, with nothing else in the payload to reveal the error. Whether LHDN would accept and file it is **unverified** — no wrong-type document has been submitted to the sandbox, and doing so deliberately is not worth the record it would create. Treat it as the most dangerous silent failure in the library regardless: the cost of being right about this is far higher than the cost of the guard.
